...
| Файл | Назначение |
| --- | --- |
| `check_server_health.sh` | Общая сводка о сервере: серийники, редакция и версия прошивки, порты, температуры, RAM и SMART |
...
| `check_emmc_health.sh` | Диагностика eMMC v2.0: SMART, несколько разделов, inodes, бенчмарки скорости и 4K задержки БД |
| `check_can_bus.sh` | Диагностика CAN/Bus77: здоровье контроллера, инвентаризация устройств (`--scan-only`), экспорт JSON |
| `monitor_can_bus.sh` | Анализатор Bus77 v2.2: декодирование трафика, расчет нагрузки (Bus Load %), Top Talkers, Ping RTT, фильтры |
