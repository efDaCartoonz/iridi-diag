...
# Standalone iRidi Pro CN cloud diagnostic.
# Usage: sh check_iridi_pro_cn.sh [--quality]
...
LOG_FILE="$LOG_DIR/cloud_iridi_pro_cn_${HOST_LABEL}_${TIMESTAMP}_$$.log"

# Open File Descriptor 3 for technical log
if ! exec 3>>"$LOG_FILE" 2>/dev/null; then
  LOG_FILE="${TMPDIR:-/tmp}/cloud_iridi_pro_cn_${HOST_LABEL}_${TIMESTAMP}_$$.log"
...
probe_resource auth-cn "Authorization CN (Global)" "https://auth.eu.iridi.com/" "95.216.162.71" yes
